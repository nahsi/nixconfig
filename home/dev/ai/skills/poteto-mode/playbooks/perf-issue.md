...
1. Capture a baseline trace from the real runtime with OMP's available process, debugger, profiler, or terminal tools.
...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to `agent: "poteto-agent"`; review the diff. Capture a post-fix trace.
