...
2. [`how`](skill://how) to ground hypotheses; don't claim a perf ceiling without running it first.
...
3. Plan the fix from the trace. If it crosses a function boundary, [`architect`](skill://architect) first. Delegate implementation through the active adapter's **Bounded session** protocol with canonical `implementer` role, or do it yourself if delegation is unavailable.
   Apply the [**sequence-verifiable-units**](skill://principle-sequence-verifiable-units) principle skill, verifying each attempt before trying the next.
4. Parse and compare the artifacts (JSON to sqlite, diff). "Inconclusive" or wrong-surface is not a pass; flag it.
5. If delivery includes a PR, run **Opening a PR**.
