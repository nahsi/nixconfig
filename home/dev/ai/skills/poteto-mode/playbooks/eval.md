...
4. **Spawn N parallel candidates** through the OMP agents selected by the **arena** skill's Phase B. Each works in its own sanitized dir; same prompt to each.
5. **Spawn one blinded OMP `reviewer`** per the **arena** skill's Phase C. Prefer a resolved model different from the candidate model. The judge sees outputs by sanitized label and the rubric, never by model name.
