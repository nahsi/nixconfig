...
4. **Spawn N parallel candidates** as built-in OMP `task` agents with `isolated: true`, following the **arena** skill's Phase B. Use the same configured task model and the same prompt for each.
5. **Spawn one blinded OMP `task` agent** per the **arena** skill's Phase C. The judge sees outputs by sanitized label and the rubric, never agent identities.
6. **Verify the chain from transcripts, not self-report.** Read each candidate's OMP transcript from the parent session's artifact directory or through `history://<agent-id>`. Stay inside the active session and workspace records. Look at which files each candidate actually opened. Citing a principle is not reading its leaf skill, and reading it is not applying it. Grade chain-following from the files it really read and the actions it took.
7. **Read every candidate output yourself** end to end. Compare to the judge's verdict. Disagreement means the rubric or one assessment is weak. Synthesize.
