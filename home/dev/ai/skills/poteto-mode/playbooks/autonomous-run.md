...
2. Pick the wake mechanism through OMP. An event to watch (CI, a merge, a ref advancing) gets a managed watcher process or agent through `hub`, with a long time-based heartbeat as fallback. No event gets a fixed-interval heartbeat sized to when the result is worth re-checking. Services, watchers, and processes that need later input stay under `hub`.
