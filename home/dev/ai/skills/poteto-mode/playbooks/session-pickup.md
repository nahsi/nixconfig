...
**You own the resume point. Read the prior trail, don't redo it.** For "take over this", "resume this conversation", "continue from <transcript path>", "you're taking over", "pick up where X left off", an `agent://` or `history://` handoff, a resume note, or a pushed branch you're meant to continue.
...
1. Locate the prior trail. Use an OMP transcript under `~/.omp/agent/sessions/<encoded-cwd>/`, an `agent://` or `history://` record, a `.scratch` resume note, or a pushed branch. Stay inside the named workspace unless the user explicitly expands scope. Read the metadata overview and last messages first, then scan back for decision points. Parse a long transcript in a `scout` and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** principle skill).
