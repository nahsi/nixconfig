...
1. [`how`](skill://how) over the affected subsystem.
2. [`architect`](skill://architect) for parallel design exploration. Skipping stays as `architect skipped: <reason>`; do not fold the design decision silently into implementation.
3. Answer the throughput checkpoint before delegating. It shapes the work items and ownership; it is not a set of todos:
   ...
   - **Shared mutable state.** Default to splitting the target (the [**separate-before-serializing-shared-state**](skill://principle-separate-before-serializing-shared-state) principle skill). Serialize only for real invariants.
...
6. Rebase into small, ordered commits; stack follow-ups.
   Use the [**sequence-verifiable-units**](skill://principle-sequence-verifiable-units) principle skill, building, verifying, and committing each small unit before the next.
7. If the design is contested, [`interrogate`](skill://interrogate) before shipping.
