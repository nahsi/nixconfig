...
1. Capture the live signal on the matching real surface with OMP's available runtime tools: a CPU profile for a spinning process, a heap snapshot for a leak, or a trace for a visual glitch. Use `hub` for managed processes and `debug` for live program state. A real artifact, not a guess.
