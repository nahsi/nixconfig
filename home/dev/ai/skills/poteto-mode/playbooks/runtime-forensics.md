...
2. Reduce the artifact to the smoking gun: the function on the hot path, the retainer chain from the leaked object to a GC root, the loop firing without input. The root starts one canonical `explorer` through the active adapter's **Bounded session** protocol with the fixed artifact and output contract. Keep only the reduced finding in the main thread (the [**guard-the-context-window**](skill://principle-guard-the-context-window) principle skill).
