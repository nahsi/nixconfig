...
State your read of scope and constraints in one paragraph. Use the structured OMP `ask` tool only for genuinely ambiguous intent (the **never-block-on-the-human** principle skill); give concrete options with each open question.
...
- Use `scout` for exploratory codebase research and `librarian` for external source research. Use `task` only when the slice also owns implementation.
- OMP resolves models through each agent's configured role. Do not hardcode model slugs in the plan.
...
If a phase creates or edits a skill, the phase instructs the implementer to read and apply `skill://writing-for-agents`, including its `SKILL-MECHANICS.md` reference.
...
**Runtime.** Exercise the feature on the matching real surface with OMP's available tools:

- Browser, Electron, or Web UIs: use the actual browser or computer tool when enabled.
- CLIs and TUIs: run the real program in a managed `hub` process or PTY and inspect its output and state.
- Native mobile: use the available simulator-driving tool.
- No runtime tool for the touched surface: flag it in the plan and name the closest observable check.
...
- `skill://deslop` over each diff before commit. the **unslop** skill over any prose surface.
