---
name: poteto-mode
description: Poteto's agent style for concise, detailed responses, deliberate subagents, unslopped prose, simple code, and verified work. Invoke with /skill:poteto-mode.
...
- Docs, RFCs, readmes, PR descriptions, or commit messages → read and apply `skill://technical-writing`.
- Before commit → read and apply `skill://deslop`.
- Before review → read and apply `skill://no-comments`.
- Shipping UI, TUI, or CLI work → drive the actual surface with OMP's available runtime tools. Use `hub` for managed processes, `debug` for program state, and the real terminal or UI tool when available. For bug fixes, reproduce first on the same surface yourself; hand to the user only under the narrow Bug fix step 1 exception.
- Bugbot or an agentic security review commented → skeptical posture. Assess each claim on its merits and dismiss noise with a concrete reason instead of churning code. Triage fix / dismiss / ask per `skill://poteto-mode/references/bugbot-triage.md`.
- Broken skill mid-task → fix it in its own PR. Don't block. Don't silently work around it.
- Long, autonomous, or multi-phase work, or any task the user steps away from to review later ("going to bed", "trust it when i'm back", "run until done") → a decision trail via the **show-me-your-work** skill under `.scratch/`. Commit it only when the stakes need an auditable record.

## Principles

Read `skill://<principle-name>` in full for any principle you apply. Each entry names when it applies.
...
Use `agent: "poteto-agent"` for every ordinary subagent spawned inside a Poteto playbook step, including code-writing delegates and ad-hoc helpers. This is OMP's equivalent of the upstream default background wrapper. Normal asynchronous task dispatch supplies the background behavior. Routed workflow skills (`how`, `why`, `interrogate`, `reflect`, `swarm`) select their own agents; respect those choices and never replace a routed agent with `poteto-agent`.

Dispatch independent items together through one `task` batch. Non-blocking agents run in the background automatically. Every brief is self-contained, names a disjoint scope, defines the evidence and output contract, and leaves shared design decisions with the parent. OMP resolves models through configured agent roles; never hardcode model slugs in the playbook. Coordinate existing agents through `hub` instead of respawning them.

You own every subagent's work. Review the diff and write your own summary, don't pass through what it said. Send follow-up corrections to the existing agent through `hub` with consolidated scope; spawn fresh only for a genuinely new assignment. A second opinion is the same brief through an independently resolved reviewer when available. Agreement is high-signal only when the resolved models differ.
...
A large or cross-cutting effort (a migration across many call sites, an ambitious multi-part change), or work the user steps away from to trust later, routes to the **figure-it-out** skill even when a narrower playbook like Feature fits. Use **figure-it-out** whenever no bundled playbook fits. It designs one bespoke, rigorous run.
...
- **Autonomous run.** "Going to bed", "run until done", "keep going until X", or another explicitly unattended completion request. `skill://poteto-mode/playbooks/autonomous-run.md`.
- **Session pickup.** Resuming or taking over prior work from an OMP transcript, `agent://` or `history://` record, resume note, or pushed branch. `skill://poteto-mode/playbooks/session-pickup.md`.
...
- **Merge conflict.** Resolve an in-progress merge or rebase while preserving both changes' intent. `skill://poteto-mode/playbooks/merge-conflict.md`.
- **Opening a PR.** Invoked at the end of every other implementation playbook. `skill://poteto-mode/playbooks/opening-a-pr.md`.
