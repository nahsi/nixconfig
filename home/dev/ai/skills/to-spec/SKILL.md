---
name: to-spec
description: Turn the current conversation into a spec, saved to `.work/` — no interview, just synthesis of what you've already discussed.
...
3. Write the spec using the template below, then save it to `.work/<slug>/spec.md`, where `<slug>` is a short kebab-case name for the work (reuse the directory if one already exists for this effort). Create the directory if needed — `.work/` is committed to version control; it is the project's shared planning memory. Tell the user the path.
