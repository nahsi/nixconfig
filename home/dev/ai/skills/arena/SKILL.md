...
Before dispatch, the lead's `todo` holds work on the requested artifact per poteto-mode's Playbooks section, not one item per Arena phase.
...
Fold each graft in by hand, per the [**redesign-from-first-principles**](skill://principle-redesign-from-first-principles) principle skill. Don't paste mechanically. The result has to remain coherent under one mental model.
...
The synthesized artifact has to hold up under the same scrutiny as any other output, per the [**prove-it-works**](skill://principle-prove-it-works) principle skill.
