...
0. **Locate the target.** Find the verification skill to maintain: the project-local skill whose body has launch/drive sections and a feature map (usually `.omp/skills/verify-*/`). Several candidates → ask which one; none → stop and point at [`/create-verification-skill`](skill://create-verification-skill) instead of inventing a target.
