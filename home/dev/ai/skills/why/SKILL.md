...
2. The category playbook linked from `skill://why/references/source-playbook.md` for the selected MCP
...
- The category playbooks linked from `skill://why/references/source-playbook.md`. One self-contained example playbook per category, plus cross-cutting `incident-postmortem.md`. Give an investigator the single file that matches its category and adapt it to the available MCP.
