...
4. Resolve workers through `skill://pstack-omp`. A model race uses discovered agents with distinct configured models.
5. Give each worker its own writable output when it writes.

## Phase B: Fan out

Spawn all N workers through `skill://pstack-omp`. Give concurrent writers disjoint paths or separate worktrees.
