...
Use `skill://writing-for-agents/SKILL-MECHANICS.md` to inspect branch coverage, leading words, false-positive near misses, and whether the skill should be model- or user-invoked. For an independent critique, select the read-only review profile through `skill://pstack-omp`, explicitly assign that reference and the query-set lens, and have it return data for coordinator synthesis. Label the result qualitative; do not report a measured trigger rate.
