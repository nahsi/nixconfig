...
This installation uses Ketch for web discovery. Prefer Ketch MCP tools when present; otherwise use the Ketch CLI. Provider failures stay inside Ketch.

- Routine search uses the configured healthy Brave default: omit `backend`.
- On `[upstream]`, use `random: ["exa", "keenable", "firecrawl"]`.
- For deep or contested research, use `multi: ["brave", "exa", "keenable", "firecrawl"]`.
- For independent-web intent — personal sites, obscure or niche subjects, older technical writing, non-commercial sources, or alternative viewpoints — call SearXNG explicitly with `!mar` and concise English keywords. Never send `!mar` to another backend.
- Use native `read` for an ordinary known URL; use Ketch `scrape` for JS rendering, batches, configured cookies, or selectors.
- Use Ketch `code` for public OSS and local tools for local/private code. Use Ketch `docs` for library discovery, native `read` for a known official documentation page, and Ketch `crawl` for multi-page site exploration.
...
search {query: "Go iter.Seq real-world experience gotchas", multi: ["brave", "exa", "keenable", "firecrawl"], limit: 5}
  → 5 rank-fused results from 4 providers
search {query: "Go range-over-func adoption production", multi: ["brave", "exa", "keenable", "firecrawl"], limit: 5}
...
| An ordinary URL you already hold | native `read` | `search` — never re-find a known URL |
| A known URL needing JS rendering, batching, configured cookies, or selectors | `scrape` | native `read` when those capabilities are required |
...
**GOOD:** Use `random: ["exa", "keenable", "firecrawl"]` so Ketch shuffles the fallback pool and stops after the first success.
...
In scope: the five research surfaces over both transports, the research and setup verbs, token budgets, error-prefix control flow, backend configuration, and cookie-authenticated fetching when the operator configured a jar. Out of scope: local or private codebase search (use repo tools), interactive authentication, CAPTCHA-protected pages, paywalls, bulk archival crawling beyond the caps, and browser automation beyond ketch's headless-rendering fallback.
