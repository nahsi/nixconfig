...
Verified against ketch v0.13.0. Discipline 6 applies: `--help`, MCP tool schemas, and `ketch config` outrank this file.
...
| `--random[=list]` | `random` (array; `["all"]` = every usable) | search; shuffled sequential fallback, mutually exclusive with `backend` and `multi`; CLI needs the `=` form for a list |
| `--multi[=list]` | `multi` (array; `["all"]` = every usable) | search; federated RRF search, mutually exclusive with `backend` and `random`; CLI needs the `=` form for a list |
...
- Backends: `brave` (default when unconfigured; requires an API key and active subscription), `ddg` (zero setup; rate-limits readily under fan-out), `searxng` (self-hosted; needs a JSON-enabled instance — see the setup verb), `exa` (zero config), `firecrawl` (Firecrawl v2 search API; needs `firecrawl_api_key`), `keenable` (keyless by default; optional `keenable_api_key` lifts the rate limit).
...
- `--random` / `random: [...]`: shuffled sequential fallback — providers are tried until one succeeds. Bare `--random` / `["all"]` uses every usable backend; an explicit list limits the pool. MCP returns the selected `backend` and an additive `errors` map for earlier failures. Mutually exclusive with `backend` and `multi`.
- The configured SearXNG instance exposes Marginalia through the `!mar` bang. Marginalia is keyword-oriented and currently English-only; use `backend: "searxng"` with `!mar` plus concise English keywords for personal, obscure, niche, older, non-commercial, or alternative web sources. Never send `!mar` to another backend.
...
- `cookie_file` points to an optional, populated Netscape-format `cookies.txt` jar for authenticated fetches. An empty file provides no authentication; interactive login, CAPTCHAs, and paywalls remain out of scope.
...
- Keys (from README and `ketch config` output): `backend`, `code_backend`, `docs_backend`, `limit`, `searxng_url`, `sourcegraph_url`, `brave_api_key`, `context7_api_key`, `github_token`, `exa_api_key`, `firecrawl_api_key`, `keenable_api_key`, `browser`, `cookie_file`, `cache_ttl`, `url_rewrites`, `spa_markers`.
- `KETCH_CONFIG=<path>` redirects every config read and write, including `ketch config set` and `ketch config path`.
