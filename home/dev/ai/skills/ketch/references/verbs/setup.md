...
2. **brave — best quality per unit effort. Recommend for regular use when already configured.** Requires an API key and active subscription; check Brave's current pricing and payment requirements before signup.
   ```sh
...
3. **searxng — the self-hosted option.** No SearXNG API key, but upstream engines can still rate-limit requests. **Stock searxng in docker rejects `format=json`**, which ketch requires — mount a `settings.yml` with the json format enabled:
