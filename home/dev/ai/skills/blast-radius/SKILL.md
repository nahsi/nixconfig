...
Companion to [`how`](skill://how) and [`why`](skill://why). [`how`](skill://how) tells you what the code does. [`why`](skill://why) tells you why it's shaped that way. Blast radius tells you what it breaks somewhere else.
...
1. Read the change. The diff, the symbols it adds, changes, and deletes, and what it now does differently, including the part the diff doesn't spell out. Use [`why`](skill://why) step 2 to pull the PR and commits.
...
4. Be honest about each risk. Give it a real chance of happening and a real cost if it does. Keep the risks you confirmed. List the ones you checked and cleared separately. Same rules as [`why`](skill://why). Cite a real `file:line`, a search that finds nothing is still an answer, and never make up a caller or an API.
...
6. For a big or wide change, run it as an [`arena`](skill://arena). Ask several models the same question and merge the answers. Different models catch different real bugs.
...
Write it through [`unslop`](skill://unslop), cite real code, and strip anything private before it goes anywhere public.
