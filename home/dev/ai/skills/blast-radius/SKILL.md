...
Companion to `skill://how` and `skill://why`. How explains what the code does. Why explains why it is shaped that way. Blast radius finds what it breaks elsewhere.
...
1. Read the change: the diff, changed symbols, and behavior the patch does not spell out. Read `skill://why` and use its code-archaeology step to pull the PR and commits.
...
4. Be honest about each risk. Give it a real chance of happening and a real cost if it does. Keep confirmed risks and list cleared ones separately. Apply the evidence rules from the Why skill already loaded. Cite a real `file:line`; a search that finds nothing is still an answer.
...
6. For a big or wide change, read and apply `skill://arena`. Use repeated OMP `task` runs as independent samples and merge the evidence; do not claim model diversity.
...
Read and apply `skill://unslop` to the writeup, cite real code, and strip anything private before public output.
