...
The scout reads the implementation and returns structured findings. The parent applies `skill://how/references/explainer-prompt.md` and writes the human-facing explanation directly from those findings.
...
After the explanation is complete, spawn two OMP `task` agents in one batch. Add `security-reviewer` or `designer` only when that specialized lens applies.

Give both task critics the same read-only brief. They are independent review contexts using the configured `task` role, not model-diverse reviewers.
...
Read `skill://interrogate` and apply its lead-judgment framework. Be a pragmatic lead, not an aggregator.
