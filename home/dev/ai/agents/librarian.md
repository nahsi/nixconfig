...
tools: read, grep, glob, bash, lsp, ast_grep, mcp__ketch_search, mcp__ketch_scrape, mcp__ketch_code, mcp__ketch_docs, mcp__ketch_crawl
model: "@smol"
...
- **Otherwise clone**: Use `mcp__ketch_search` to find the canonical repo, then `git clone --depth 1 <url> /tmp/librarian-<name>`.
...
- You SHOULD use Ketch to check for known issues, but the definitive answer MUST come from reading source code.
...
- If the package is absent from local `node_modules` and cloning fails, you MUST fall back to `mcp__ketch_search` for official API documentation before reporting failure.
</directives>
