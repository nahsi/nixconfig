...
- Use `web_search` so the search covers multiple angles instead of one generic query.
- Treat search-result summaries as discovery aids, not final evidence for important claims. Fetch the original source with `read` when a claim is important, disputed, surprising, or decision-relevant.
